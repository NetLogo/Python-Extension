...
to setup
  ...
  py:set "hl_size" hidden-layer-size
  py:set "num_actions" 3
  py:set "discount" discount
  py:set "lr" lr

  (py:run
    "sheep_model = Sequential()"
    "sheep_model.add(Dense(hl_size, input_shape=(state_size,), activation = 'relu'))"
    "sheep_model.add(Dense(hl_size, activation='relu'))"
    "sheep_model.add(Dense(num_actions))"
    "sheep_model.compile(sgd(lr=lr), 'mse')"
    "sheep_model.summary()"
  )
...
to eat-sheep  ; wolf procedure
  ...
  if prey != nobody  [                          ; did we get one?  if so,
    ask prey [ sheep-death ]                            ; kill it, and...
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot loss"
